...
(Previously: Every provider mutation called `await _storage.save(_favorites)` synchronously
inside the public method, blocking the UI frame.)

The provider SHALL schedule a storage write 350 ms after the last mutation. The provider
SHALL expose `flush()` and SHALL call it from `dispose()` and on
